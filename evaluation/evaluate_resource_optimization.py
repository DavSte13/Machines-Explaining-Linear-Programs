...
def evaluate_basic_lp(evaluation_function, input_parameters, attribution_method, baselines, descriptions):
    ...
    for i in range(num_runs):
        ...
        if attribution_method[i] == 'grad':
            attr = xlp_utils.detach_tuple(torch.autograd.functional.jacobian(blp, input_parameters[i]), to_list=True)
            ...
            attr = xlp_utils.detach_tuple(gc.attribute(input_parameters[i]), to_list=True)
